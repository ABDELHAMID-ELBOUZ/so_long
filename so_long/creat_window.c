...
/*   Updated: 2025/02/01 16:15:01 by aelbouz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	creat_window(t_game *game, int rows, int columns)
{
	game->mlx = mlx_init();
	game->win = mlx_new_window(game->mlx, rows * 64, columns * 64, "so_long");
	if (!game->win)
	{
		free(game->win);
		exit(0);
